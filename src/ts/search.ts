...
if (document.querySelector("#search-container")) {
  const observer = new MutationObserver((mutations, observer) => {
    ...
    if (mutation && mutation.addedNodes && mutation.addedNodes.length > 0) {
      ...
      const hasLoadedSwiftype =
        newNode &&
        typeof newNode.getAttribute === "function" &&
        newNode.getAttribute("id") === "st-injected-content";

      if (hasLoadedSwiftype) {
        // Find our results container and reparent the Swiftype container with it.
        var resultsContainer = document.querySelector("#search-results");
        if (resultsContainer) {
          const autocompleteSuggestionBox = document.querySelector(
