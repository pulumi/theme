...
function selectChoice(kind, choice, extra?) {
  ...
  document.cookie =
    "pulumi_" + kind + "=" + choice + "; max-age=31536000; path=/";
...
function selectLanguage(lang) {
  selectChoice("language", lang, function () {
    ...
    $("code").each(function (i, e) {
      ...
      for (var i = 0; i < classes.length; i++) {
        if (
          classes[i].startsWith("language-") &&
          shellLanguages.indexOf(classes[i]) === -1
        ) {
          ...
          if (
            lang === "javascript" &&
            (classes[i] === "language-typescript" ||
              classes[i] === "language-ts")
          ) {
...
$(document).on("rendered", function () {
  ...
  ["language", "os", "cloud", "k8s-language", "input-kind"].forEach(function (
    kind
  ) {
