...
export function waitForElementToExist(
  ...
  return new Promise((resolve, reject) => {
    ...
    setTimeout(() => {
      ...
      return reject(
        new Error(
          `The provided selector [${selector}] was not found on the DOM after ${timeout}ms.`
        )
      );
    }, timeout);
  });
...
// variable.
export function waitForWindowPropertyToExist(
  key: string,
  timeout: number = 3000
) {
  return new Promise((resolve, reject) => {
    ...
    setTimeout(() => {
      ...
      return reject(
        new Error(
          `The provided key [${key}] didn't propogate after ${timeout}ms.`
        )
      );
    }, timeout);
  });
