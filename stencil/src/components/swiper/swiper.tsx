...
export class Swiper {
  ...
  @State()
  swiperID =
    Math.random().toString(5).substring(2, 15) +
    Math.random().toString(5).substring(2, 15);
  ...
  componentDidLoad() {
    ...
    const swipeContainer = this.el.querySelector(
      `.swiper-container.${this.containerClass}`
    ) as HTMLElement;
    ...
    if (this.autoplay) {
      ...
      if (this.enableMouseEvents) {
        swipeContainer.addEventListener(
          "mouseenter",
          this.stopSwiper.bind(this)
        );
        swipeContainer.addEventListener(
          "mouseleave",
          this.startSwiper.bind(this)
        );
      }
    }
  }
