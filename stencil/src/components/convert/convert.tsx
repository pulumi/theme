...
export type OutputEditorFilename =
  | "index.ts"
  | "index.js"
  | "__main__.py"
  | "main.go"
...
export class Convert {
  ...
  private validateProps() {
    ...
    if (!this.theme) {
      console.warn(
        "A CodeMirror theme was not specified. The Using `material-ocean`."
      );
    }
  ...
  private prepareEditors() {
    ...
    this.inputEditor.on(
      "keydown",
      (_instance: CodeMirror.Editor, event: KeyboardEvent) => {
  ...
  private async convert() {
    ...
    try {
      const response = await fetch(
        [this.endpointURL, this.endpointPath].join("/"),
        {
          method: "POST",
          mode: "cors",
          body: JSON.stringify({
      ...
      if (result.error) {
        ...
        if (!filename.match(/\w+\.\w+/)) {
          filename = this.supportedLanguages.find(
            (sl) => sl.key === language.key
          ).filename;
        }
        ...
        const diagnostics =
          result.diagnostics?.replace(/^no diagnostics$/, "") || "";
  ...
  private renderDismissAlertButton() {
    return (
      <button
        ...
        onClick={this.dismissAlert.bind(this)}
      >
        <span class="icon"></span>
      </button>
    );
        ...
        return (
          <div class="status-bar">
            <span class="message">
              ...
              {this.selectedSourceFile?.description
                ? ` • ${this.selectedSourceFile.description}`
                : ""}
            </span>
          </div>
          ...
          <div class={this.statusBarClasses}>
            ...
            <div
              class={this.combineClasses(
                "alert",
                "alert-error",
                this.alertDismissed ? "dismissed" : undefined
              )}
            >
              ...
              <p>
                There could be a problem with the code you submitted, or it
                might use a feature {this.conversionTool.name} doesn't support.
                Join us in{" "}
                <a href="https://slack.pulumi.com/">Community Slack</a> for
                help.
              </p>
            </div>
            <div
            ...
            >
              ...
              <p>
                <strong>
                  Sorry, we were unable to convert your code completely.
                </strong>
              </p>
              <p>
                The code was valid, but {this.conversionTool.name} was unable to
                convert it completely. There could be a problem with the code
                you submitted, or it might use a feature{" "}
                {this.conversionTool.name} doesn't support. Join us in{" "}
                <a href="https://slack.pulumi.com/">Community Slack</a> for
                help.
              </p>
            </div>
        ...
        <div id="editors">
          <div class="editor editor-input">
            <div class="editor-heading">
              ...
              <p>
                We've included a few examples for reference &mdash; feel free to
                edit them as you like, or replace them with your own code. (And
                don't worry, we send your code over SSL and don't store anything
                on our servers.) When you're ready to transform your{" "}
                {this.sourceLanguageName} code to Pulumi, select{" "}
          ...
          >
            <div class="editor-heading">
              ...
              <p>
                ...
                {this.conversionTool.githubURL ? (
                  <a href={this.conversionTool.githubURL}>
                    {this.conversionTool.name}
                  </a>
                ) : (
                  this.conversionTool.name
                )}
                , an open-source command-line tool we built to make it as easy
                as possible for you to migrate your existing{" "}
                {this.sourceLanguageName} projects to Pulumi. The resulting file
                can be copied or downloaded for use with{" "}
