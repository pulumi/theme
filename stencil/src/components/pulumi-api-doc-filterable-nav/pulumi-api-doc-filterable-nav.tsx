...
export class PulumiApiDocFilterableNav {
    constructor() {
        ...
        this.debouncedFilterTree = debounce(this.debouncedFilterTree, 100);
    }
    ...
    async loadNavigationData() {
        ...
        try {
            const response = await fetch(
                `/registry/packages/navs/${this.packageName}.json`
            );
    ...
    filterTreeToMatchingRecursive(
        ...
        nodesToSearch.map((node) => {
            ...
            const isParentDuplicate = directParentNode
                ? !!nodeCache[parentName]
                : false;
        ...
        reconstructedTreeOfMatches.map((root) => {
            root.children = nodesToRender.filter(
                (nodeToRender) => nodeToRender.parentName === root.name
            );

            root.children?.map((child) => {
                child.children = nodesToRender.filter(
                    (nodeToRender) => nodeToRender.parentName === child.name
                );
            });
        });
            ...
            <div class="filter-and-nav-tree">
                ...
                {this.filterContent?.length === 1 && (
                    <div class="filter-help-text">
                        Provide at least two characters to filter.
                    </div>
                )}
