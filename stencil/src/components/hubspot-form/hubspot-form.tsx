...
export class HubspotForm {
  ...
  private onMessage(event: MessageEvent) {
    ...
    if (eventName === "onFormReady") {
      ...
      const utmCampaignInput: HTMLInputElement = this.el.querySelector(
        `input[name="last_utm_campaign"]`
      );
      if (utmCampaignInput) {
        utmCampaignInput.value = utmData.campaign;
      }

      const utmSourceInput: HTMLInputElement = this.el.querySelector(
        `input[name="last_utm_source"]`
      );
      if (utmSourceInput) {
        utmSourceInput.value = utmData.source;
      }

      const utmMediumInput: HTMLInputElement = this.el.querySelector(
        `input[name="last_utm_medium"]`
      );
      if (utmMediumInput) {
    ...
    if (eventName === "onFormSubmit") {
      const emailAddress: HTMLInputElement =
        this.el.querySelector(`input[name="email"]`);
  ...
  private notifySegment(emailAddress: string, utmData: UTMData) {
    ...
    const analyticsAvailable =
      analytics && analytics.track && typeof analytics.track === "function";
  ...
  private setInternalAdId() {
    ...
    if (internalAdId) {
      const internalAdIdInput: HTMLInputElement = this.el.querySelector(
        `input[name="last_internal_ad_conversion"]`
      );
  ...
  private renderFailedToLoadForm() {
    return (
      <p>
        ...
        browser, and if you continue to see this message, let us know at{" "}
  ...
  render() {
    return (
      <div id={this.hubspotFormTargetId}>
        {!this.didLoad && !this.isLoading
          ? this.renderFailedToLoadForm()
          : this.renderIsLoadingForm()}
      </div>
    );
  }
}
